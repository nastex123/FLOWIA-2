...
| **P4a — Beatriz** | Automatización Hot-Folder (agente de bandeja → backend) | Hot-folder envía a `/upload` con API Key y notifica nativamente; `settings_view.py` integrado; suite completa en verde. | ✅ **COMPLETADO** |
| **P4b — Hector** | Integración Final & Orquestación (coordinador del vertical) | Vertical integrado y verificado end-to-end; documentación sincronizada y CHANGELOG consolidado. | ✅ **COMPLETADO** |