"""Native PySide6 Desktop User Interface for FlowMind AI (Glassmorphism Cyber-Dark)."""
...
class MainWindow(QMainWindow):
    """FlowMind AI Desktop Main Application Window with Collapsible Glass Sidebar."""

    def __init__(self, client: Optional[DesktopFlowMindClient] = None):
        super().__init__()
        self.setWindowTitle("FlowMind AI — Suite de Gestión Financiera & Auditoría Local")
    ...
    def _init_ui(self) -> None:
        ...
        # 2. Central Layout & Collapsible Sidebar
        ...
        self.sidebar_title = QLabel("<b style='color: #cbd5e1; font-size: 14px;'>NAVEGACIÓN</b>")
        ...
        # Sidebar Footer
        self.sidebar_footer = QLabel("<div style='color: #475569; font-size: 11px; text-align: center;'>FlowMind AI v0.2.0<br>100% Zero Cloud</div>")
        self.sidebar_footer.setAlignment(Qt.AlignmentFlag.AlignCenter)
        sidebar_layout.addWidget(self.sidebar_footer)

        main_layout.addWidget(self.sidebar_container)

        # 3. Main Stacked Views
        ...
        ph_label = QLabel("<h3>🔍 Ningún comprobante seleccionado</h3><p style='color: #94a3b8;'>Ve a <b>Facturas & Auditoría</b> y haz doble clic sobre cualquier documento para abrir la inspección de auditoría.</p>")
        ...
        self.settings_label = QLabel("<h3>⚙️ Configuración del Sistema</h3><p style='color: #94a3b8;'>El módulo completo de configuración del agente Hot-Folder se integrará en P4.</p>")
    ...
    def _toggle_sidebar(self) -> None:
        """Toggles between expanded (240px) and collapsed (68px) sidebar."""
        ...
        if self._sidebar_expanded:
            ...
            self.sidebar_container.setFixedWidth(68)
    ...
    def _build_local_view(self) -> QWidget:
        ...
        left_layout.addWidget(QLabel("<b>Campos Clave Extraídos & Metadatos Locales</b>"))
        ...
        if self._settings_widget is None:
            try:
                ...
                self.settings_label.setText(
                    "<h3>⚙️ Configuración del Hot-Folder</h3><p style='color: #94a3b8;'>El módulo `SettingsView` se integrará completamente en la fase P4.</p>"
                )
    ...
    def _prompt_login(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.status_bar.showMessage(
                f"Conectado exitosamente a {self.client.base_url} — Org: {org}"
            )
        else:
            self.conn_pill.setText("● Modo Offline")
            self.conn_pill.setStyleSheet(get_badge_qss("warning"))
            self.status_bar.showMessage("Sin autenticar — Modo Demo / Extracción Local activo.")
    ...
    def process_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            lines = [
                ...
                f"⏱️ Tiempo de Inferencia: {res.get('processing_time_ms', 0)} ms",
                "================================================",
                "CAMPOS EXTRAÍDOS E IDENTIFICADOS:",
            ]
            ...
            if tables:
                ...
                self.status_bar.showMessage(
                    f"Extracción completada en {res.get('processing_time_ms', 0)}ms — {len(records)} filas procesadas."
                )
            else:
                self.table_model.set_data([], [])
                self.status_bar.showMessage(
                    f"Extracción completada en {res.get('processing_time_ms', 0)}ms — Sin tablas detectadas."
                )
